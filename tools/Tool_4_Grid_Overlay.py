...
- Saves current grid only: image -> output/, settings -> data/
"""
...
def run(image_path: str) -> int:
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            # Use simple naming without grid size
            json_path = os.path.join(data_dir, f"{base_name}_grid.json")
            with open(json_path, 'w') as f:
                json.dump(grid_data, f, indent=2)

            print(f"✓ Saved current grid: {cols}x{rows} cells")
            print(f"  Image: {os.path.relpath(img_path, project_root)}")
            print(f"  Settings: {os.path.relpath(json_path, project_root)}")
