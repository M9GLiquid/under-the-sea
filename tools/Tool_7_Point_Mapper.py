...
class PointMapperApp:
    def __init__(self, fisheye_json: str, transform_json: str, server_w: int = 2048, server_h: int = 1536, auto_detect_size: bool = False, scale_factor: float = 0.8):
        ...
        if auto_detect_size:
            # Use the actual loaded image dimensions for API usage
            self.server_size = (self.img_original.shape[1], self.img_original.shape[0])
            print(f"Auto-detected server image size: {self.server_size[0]}x{self.server_size[1]}")
        else:
            # Use manual input for testing/development
            self.server_size = (int(server_w), int(server_h))
            print(f"Using manual server image size: {self.server_size[0]}x{self.server_size[1]}")
...
def main() -> int:
    ...
    parser.add_argument("--server-width", type=int, default=2048, help="Server original image width (e.g., 2048). Ignored if --auto-detect-size is used.")
    parser.add_argument("--server-height", type=int, default=1536, help="Server original image height (e.g., 1536). Ignored if --auto-detect-size is used.")
    parser.add_argument("--auto-detect-size", action="store_true", help="Automatically detect server image size from loaded original image (for API usage)")
    ...
    try:
        app = PointMapperApp(
            ...
            auto_detect_size=args.auto_detect_size,
